    ...
    <div style={{ display:'grid', gap: 16 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: t.font.sm, color: t.color.textDim, marginBottom: 6 }}>הרגלים היום</div>
          <div style={{ fontSize: t.font.xxl, fontWeight: 800, color: t.color.gold }}>{doneToday}/{state.habits.length}</div>
        </Card>
        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: t.font.sm, color: t.color.textDim, marginBottom: 6 }}>סה״כ streaks</div>
          <div style={{ fontSize: t.font.xxl, fontWeight: 800, color: t.color.success }}>{totalStreak} 🔥</div>
        </Card>
      </div>

      <Card>
        <SectionHeader title="ההרגלים שלי" action={<Button onClick={() => setOpen(true)}>+ הרגל חדש</Button>} />
          ...
          {state.habits.map(h => (
            <div key={h.id} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: t.font.xs, color: t.color.textDim }}>🔥 {h.streak} ימים ברצף</div>
              </div>
              <Button variant="ghost" size="sm" onClick={() => removeHabit(h.id)}>✕</Button>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        <SectionHeader title="עקביות ב-8 שבועות אחרונים" subtitle="דמו · יתמלא לפי ההרגלים שלך" />
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="הרגל חדש">
        <div style={{ display:'grid', gap: 12 }}>
          <Input label="שם ההרגל" placeholder="לדוגמה: מדיטציה 10 דק׳" value={name} onChange={e => setName(e.target.value)} />
          <Input label="אייקון" placeholder="✨" value={icon} onChange={e => setIcon(e.target.value)} />
          <div style={{ display:'flex', gap: 10, justifyContent:'flex-end' }}>
            <Button variant="ghost" onClick={() => setOpen(false)}>בטל</Button>
            <Button onClick={() => { if (name) { addHabit({ id: 'h_'+Date.now(), name, icon, streak:0, doneToday:false }); setName(''); setIcon('✨'); setOpen(false) } }}>הוסף</Button>
          </div>
        </div>
      </Modal>
    </div>
